	...
	public void onRun() {
		...
		for(int i = startIndex; i < endIndex; i++){
			...
			for(Sphere sphere2 : objects) {
				...
				double distance = Maths.getDistanceBetweenPoints(sphere.position.x, sphere.position.y, sphere.position.z, sphere2.position.x, sphere2.position.y, sphere2.position.z);
				if(distance < sphere.scale + sphere2.scale) {
					if(sphere.mass > sphere2.mass) {
