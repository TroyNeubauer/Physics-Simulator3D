...
import com.troy.ps.cltypes.CLTypes;
import com.troy.ps.world.World;
import com.troyberry.logging.Timer;
import com.troyberry.math.Maths;
import com.troyberry.opencl.*;
import com.troyberry.opengl.mesh.Vbo;
...
public class OpenCLManager {
	...
	public static void createBodies(int offset, int count, float[] prePos, float[] postPos, float[] mass, float[] radius) {
		CLMem prePosMem = device.allocateMemory(NIOBufferUtils.createFloatBuffer(prePos));
		CLMem postPosMem = device.allocateMemory(NIOBufferUtils.createFloatBuffer(postPos));
		CLMem massMem = device.allocateMemory(NIOBufferUtils.createFloatBuffer(mass));
		CLMem radiusMem = device.allocateMemory(NIOBufferUtils.createFloatBuffer(radius));
		int index = 0;
	...
	public static void setMem(Vbo posVbo, Vbo colorVbo, Vbo radiusVbo, float[] velocities) {
		...
		vel = device.allocateMemory(NIOBufferUtils.createFloatBuffer(velocities));
